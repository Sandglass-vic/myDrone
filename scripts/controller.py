...
class ControllerNode:
	...
	def decision(self):
		if self.flight_state_ == self.FlightState.WAITING:  # 起飞并飞至离墙体（y = 3.0m）适当距离的位置
			...
			if abs(dist) < 0.3:  # 当前段导航结束
				...
				command = ""
				if dim_index == 0:
					command = "right" if dir_index == 0 else "left"
				elif dim_index == 1:
					command = "forward" if dir_index == 0 else "back"
				else:
					command = "up" if dir_index == 0 else "down"
				if abs(dist) > 1.5:
			...
			if self.detectTarget():
				...
				self.navigating_queue_ = deque([['y', 2.4], ['z', 1.0], ['x', self.window_x_list_[win_index]], ['y', 5.0], ['x', 7.0]])
				self.switchNavigatingState()
	...
	def publishCommand(self, command_str):
		rospy.logwarn('Command: ' + command_str +']')
		msg = String()
